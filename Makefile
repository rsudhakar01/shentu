...
install: go.sum
	go install $(BUILD_FLAGS) ./cmd/certik
...
update-cli-docs: install
	certik --doc docs/cli/certik
	certikcli --doc docs/cli/certikcli
	@perl -pi -e "s|^#* Auto generated by .*||" docs/cli/**/*.md
	@perl -pi -e "s|$$HOME|~|" docs/cli/**/*.md

release: go.sum
	GOOS=linux go build $(BUILD_FLAGS) -o build/certik ./cmd/certik
	GOOS=windows go build $(BUILD_FLAGS) -o build/certik.exe ./cmd/certik
	GOOS=darwin go build $(BUILD_FLAGS) -o build/certik-macos ./cmd/certik

build: go.sum
ifeq ($(OS),Windows_NT)
	go build -mod=readonly $(BUILD_FLAGS) -o build/certik.exe ./cmd/certik
else
	go build -mod=readonly $(BUILD_FLAGS) -o build/certik ./cmd/certik
endif
...
localnet-start: build-linux build-docker-certikdnode localnet-stop
	@if ! [ -f build/node0/certik/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/certik:Z certikfoundation/certikdnode testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
